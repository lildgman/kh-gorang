...
function setSidebarTitle(pageType) {
    if (pageType === 'recipe' || pageType === 'main_recipe') {
        document.querySelector("#recently-seen-container > span").innerHTML = "최근 본 레시피";
    } else if (pageType === 'product' || pageType === 'main_product') {
        document.querySelector("#recently-seen-container > span").innerHTML = "최근 본 상품";
    }
}
...
function loadSideBar() {
    ...
    if (pageType === 'main_recipe') {
        ...
        console.log("onload-thumbnailUrl = ", thumbnailUrl);
...
function initRecentlyWatchedList(itemId, thumbnailUrl, ctx, isRecipe) {
    ...
    if (index === -1) {
        ...
        // 최근 본 목록에 아이템 ID 추가
        // watched.push(itemId);
        watched.push({id: itemId, thumbnail: thumbnailUrl});
